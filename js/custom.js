...
var address     = "26 west 23rd street, <br />New York, NY";
var phone       = "+1 (646) 555-8901";
var email       = "info@weareblkbox.com";
...
$(function () {
        $("#modal-save").click(function () {
            ...
            $("#hidden-access").val(val + form);
            $('#data-access-modal').modal('hide') ;
        })

    $(".data-access-select").change(function () {
        if ($(this).val() =='excel' || $(this).val() == 'other') {
            $("#excel-data-access").removeClass('hidden');
            $("#db-data-access").addClass('hidden');
        } else {
            $("#excel-data-access").addClass('hidden');
            $("#db-data-access").removeClass('hidden');
        }
        $('#data-access-modal').modal('show') ;
        $("#data-access-hidden").val($(this).attr('name'));
        $('#data-access-modal').data('type', $(this).attr('name'));
    })
    //debugger;//
    ...
    $("#datatab .radio-custom").on('click', function () {
        var hasNotSure = false;
        $("#datatab .radio-custom:checked").each(function () {
            if($(this).val().indexOf('not-sure') > -1) {
                hasNotSure = true;
                console.log($(this).val());
            }
        });
        if (hasNotSure) {
            $("#contact_slidein").show('slow')
        } else {
            $("#contact_slidein").hide('fast');
        }
    })
    ...
    $("#id_role").change(function () {
        ...
        if( $(this).val().length > 0) {
            ...
            $("#great-msg").closest(".hidden").removeClass('hidden');
            $("#great-msg").text(val);
        }
        else {
            $("#great-msg").closest(".row").parent().addClass('hidden');
        }
    });
    $("#sure").click(function () {
    ...
    var getTrackStyle = function (el) {
        ...
        // Set active label
        $('#'+target+'.range-labels li').removeClass('active selected');

        var curLabel = $('#'+target+'.range-labels').find('li:nth-child(' + curVal + ')');
    ...
    $rangeInput.on('input', function () {
        ...
        if(parseInt($(this).val()) >= 5) {
          $("#" + $(this).data('target') + "-extra").removeClass('hidden');
        }
        else {
          $("#" + $(this).data('target') + "-extra").addClass('hidden');
        }
    });
...
// Change input value on label click
    $('.range-labels li').on('click', function () {
        ...
        var domId = "id_" + $(this).closest('.range-labels').attr('id');
